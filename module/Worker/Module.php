...
class Module implements AutoloaderProviderInterface
{
    ...
    public function getServiceConfig() {
        return array(
            'factories' => array(
                'Worker\Model\WorkerTable' => function($sm) {
                    $dbAdapter = $sm->get('Zend\Db\Adapter\Adapter');
                    $table = new Model\WorkerTable($dbAdapter);
                    return $table;
                },
                'Worker\Model\WorkerOrderTable' => function($sm) {
                        $dbAdapter = $sm->get('Zend\Db\Adapter\Adapter');
                        $table = new Model\WorkerOrderTable($dbAdapter);
                        return $table;
                    },
                'Worker\Model\OrderTable' => function($sm) {
                        $dbAdapter = $sm->get('Zend\Db\Adapter\Adapter');
                        $table = new Model\OrderTable($dbAdapter);
                        return $table;
                    },
                'Worker\Model\OrderDetailTable' => function($sm) {
                        $dbAdapter = $sm->get('Zend\Db\Adapter\Adapter');
